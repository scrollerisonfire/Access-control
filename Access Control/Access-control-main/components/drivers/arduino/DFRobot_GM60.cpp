...
bool DFRobot_GM60::setIdentifyQR()
{
  uint8_t ack;
  uint8_t data = 1;

  ack = writeReg(REG_IDENTIFY_QR, &data, 1);
  if (ack == 0x02)
  {
    return true;
  }
...
bool DFRobot_GM60::cycleLEDs(uint8_t data)
{
  uint8_t ack;
  ack = writeReg(REG_LEDS_CYCLE, &data, 1);
  if (ack != 0x02)
  {
    return false;
  }
...
bool DFRobot_GM60::stopReading()
{
  uint8_t ack;
  uint8_t data = 0x80;
  ack = writeReg(REG_SUCCESS, &data, 1);
  if (ack != 0x02)
  {
    return false;
  }
...
bool DFRobot_GM60::successReadLEDs()
{
  uint8_t ack;
...
bool DFRobot_GM60::rawCommand(uint16_t reg,uint8_t data)
{
  uint8_t ack;
  ack = writeReg(reg, &data, 1);
  if (ack != 0x02)
  {
    return false;
  }
...
bool DFRobot_GM60::setLight(uint8_t data)
{
  uint8_t ack;
  ack = writeReg(REG_LIGHTS, &data, 1);
  if (ack != 0x02)
  {
    return false;
  }
...
bool DFRobot_GM60::setOnlyRedLED()
{
  uint8_t ack;
  uint8_t data = 0b11001100;
  ack = writeReg(REG_LEDS_0_1, &data, 1);
  if (ack != 0x02)
  {
    return false;
  }
  //data = 0b11001100;
  ack = writeReg(REG_LEDS_2_3, &data, 1);
  if (ack != 0x02)
  {
    return false;
  }
  ack = writeReg(REG_LEDS_4_5, &data, 1);
  if (ack != 0x02)
  {
    return false;
  }
  ack = writeReg(REG_LEDS_6_7, &data, 1);
  if (ack != 0x02)
  {
    return false;
  }
...
bool DFRobot_GM60::setOnlyGreenLED()
{
  uint8_t ack;
  uint8_t data = 0; //0b101010100;
  ack = writeReg(REG_LEDS_0_1, &data, 1);
  if (ack != 0x02)
  {
    return false;
...
bool DFRobot_GM60_UART::begin(HardwareSerial &s_)
{
  s = &s_;
  if (s != NULL)
  {
